...
mod jericho_lending_protocol 
{
    ...
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    #[ink(storage)]
    pub struct JerichoLendingProtocol 
    {
        borrow_positions: Mapping<u128, position_data>,
        lender_deposited_jericho: Mapping<AccountId, u128>,
        borrower_collateral: Mapping<AccountId, u128>,

        oracle_address: AccountId,
        jericho_stablecoin_address: AccountId,
        share_tokens_address: AccountId,
        wrapped_dot_address: AccountId,
        
        dot_str_pair: String,

        total_jericho_deposited: u128,
        available_jericho_liquidity_reserve: u128,
    ...
    
    impl JerichoLendingProtocol 
    {
        ////////////////////////////////////////////////////////////////////////////
        /////"CONSTRUCTORS"/////////////////////////////////////////////////////////
        ////////////////////////////////////////////////////////////////////////////
        #[ink(constructor)]
        pub fn new(param_oracle_address: AccountId, param_jericho_stablecoin_address: AccountId, param_share_tokens_address: AccountId, param_wrapped_dot_address: AccountId, param_dot_pair_str: String) -> Self 
        {
            ...
            Self 
            {
                ...
                lender_deposited_jericho: empty_mapping_2,
                borrower_collateral: empty_mapping_3,

                oracle_address: param_oracle_address,
                jericho_stablecoin_address: param_jericho_stablecoin_address,
                share_tokens_address: param_share_tokens_address,
                wrapped_dot_address: param_wrapped_dot_address,
                
                dot_str_pair: param_dot_pair_str,

                total_jericho_deposited: 0,
                available_jericho_liquidity_reserve: 0,
        ...
        pub fn deposit_initial_liquidity(&mut self, param_initial_liquidity: u128)
        {
            ...
            .call(self.jericho_stablecoin_address)
            .transferred_value(0)
            .exec_input(
            ...
            assert!(my_return_value >= param_initial_liquidity, "Not enough allowance. Plase permit jerichoProtocol to use the asset.");
            ...
            self.total_jericho_deposited += param_initial_liquidity;
            self.available_jericho_liquidity_reserve += param_initial_liquidity;
        }
        ...
        pub fn deposit(&mut self, param_amount_to_deposit: u128)
        {
            ...
            assert!(my_return_value >= param_amount_to_deposit, "Not enough allowance. Plase permit jerichoProtocol to use the asset.");
            ...
            let mut actual_caller_deposited_jericho_balance = self.lender_deposited_jericho.get(caller).unwrap_or(0);
            actual_caller_deposited_jericho_balance += param_amount_to_deposit;
            self.lender_deposited_jericho.insert(&caller, &actual_caller_deposited_jericho_balance);

            self.total_jericho_deposited += param_amount_to_deposit;
            self.available_jericho_liquidity_reserve += param_amount_to_deposit;

            ink::env::call::build_call::<Environment>()
            .call(self.share_tokens_address)
            .transferred_value(0)
            .exec_input(
        ...
        {
            let caller = self.env().caller();
            let user_deposited_jericho = self.lender_deposited_jericho.get(caller).unwrap_or(0);
            let actual_liquidity_reserve = self.available_jericho_liquidity_reserve;

            assert!(user_deposited_jericho >= param_amount_to_redeem, "Incorrect amount to redeem, the specified amount is bigger than the actual balance.");
            assert!(actual_liquidity_reserve >= param_amount_to_redeem, "Not enough liquidity.");


            let total_shares = ink::env::call::build_call::<Environment>()
            .call(self.share_tokens_address)
            .transferred_value(0)
            .exec_input(
                ExecutionInput::new(Selector::new(ink::selector_bytes!("get_total_supply")))
            )
            .returns::<u128>()
            .invoke();

            let user_shares = ink::env::call::build_call::<Environment>()
            .call(self.share_tokens_address)
            .transferred_value(0)
            .exec_input(
            ...
            assert!(user_shares >= param_amount_to_redeem, "Not enough shares.");
            ...
            let total: u128 = 100;
            let user_shares_percentage = (total).wrapping_mul(param_amount_to_redeem).wrapping_div(total_shares);
            ...
            let balance_plus_fees = (self.total_jericho_deposited).wrapping_mul(user_shares_percentage).wrapping_div(100);
            ...
            .transferred_value(0)
            .exec_input(
                ExecutionInput::new(Selector::new(ink::selector_bytes!("burn_tokens")))
                .push_arg(caller)
            ...
            self.total_jericho_deposited -= balance_plus_fees;
            self.available_jericho_liquidity_reserve -= balance_plus_fees;
        }
        ...
        pub fn borrow(&mut self, param_amount_dot_collateral: u128)
        {
            ...
            assert!(my_return_value >= param_amount_dot_collateral, "Not enough allowance. Plase permit jerichoProtocol to use the asset.");
            ...
            assert!(self.available_jericho_liquidity_reserve >= borrow_value, "Not enough reserve liquidity.");
            ...
            self.available_jericho_liquidity_reserve -= borrow_value;
        ...
        pub fn repay(&mut self, position_to_repay_id: u128, param_amount_to_repay: u128)
        {
            ...
            assert!(my_return_value >= param_amount_to_repay, "Not enough allowance. Plase permit jerichoProtocol to use the asset.");
            ...
            if param_amount_to_repay >= to_repay
            {
                ...
                self.available_jericho_liquidity_reserve += param_amount_to_repay;
        ...
        pub fn liquidate(&mut self, param_borrow_position_id: u128)
        {
            ...
            assert!(my_return_value >= position_info.pending_debt_without_interests, "Not enough allowance. Plase permit jerichoProtocol to use the asset.");
            ...
            self.available_jericho_liquidity_reserve += position_info.pending_debt_without_interests;
        }

        #[ink(message)]
        pub fn flashloan(&mut self,  param_amount_jericho_stablecoin_to_use: u128)
        {
            assert!(self.available_jericho_liquidity_reserve >= param_amount_jericho_stablecoin_to_use, "Not enough liquidity available for the flashloan.");

            let caller = self.env().caller();
            let contract_address = self.env().account_id();

            let balance_before = self.available_jericho_liquidity_reserve;
            let balance_after;
            ...
            .exec_input(
                ExecutionInput::new(Selector::new(ink::selector_bytes!("transfer")))
                .push_arg(param_amount_jericho_stablecoin_to_use)
                .push_arg(caller)
            )
            .returns::<()>()
            .invoke();
            ...
            let fee = param_amount_jericho_stablecoin_to_use / 10;

            assert!(balance_after >= balance_before + fee, "Liquidity not returnd correctly.");

            self.available_jericho_liquidity_reserve = balance_after;
        }

        #[ink(message)]
        pub fn get_actual_jericho_reserves(&self) -> u128
        {
            self.available_jericho_liquidity_reserve
        }
